...
def get_sl_tp(close, atr, signal, multiplier):
    ...
    if signal == "BUY":
        return round(close - sl_distance, 5), round(close + tp_distance, 5)
    elif signal == "SELL":
        return round(close + sl_distance, 5), round(close - tp_distance, 5)
    return None, None
...
with tab1:
    ...
    if timeframe in ["1 Minute", "15 Minutes"]:
        st.warning("Short timeframes (1min, 15min) only support limited history due to data provider restrictions.")
        ...
        for symbol in pairs:
            with st.spinner("Analyzing " + ALL_PAIRS.get(symbol, symbol) + " on " + timeframe + "..."):
                try:
                    ...
                    if df.empty:
                        st.error("No data returned for " + symbol + " on this timeframe. Try a different period.")
                        continue
                    ...
                    prompt = (
                        ...
                        " on the " + timeframe + " timeframe based on these indicators:\n"
                        "- Current Price: " + str(close) + "\n"
                        "- 20 MA: " + str(ma20) + "\n"
                        "- 50 MA: " + str(ma50) + "\n"
                        "- 200 EMA: " + str(ema200) + "\n"
                        "- RSI: " + str(rsi) + "\n"
                        "- BB Upper: " + str(bb_upper) + "\n"
                        "- BB Lower: " + str(bb_lower) + "\n"
                        "- MACD: " + str(macd) + "\n"
                        "- Signal: " + str(signal) + "\n"
                        "- Stoch K: " + str(stoch_k) + "\n"
                        "- Stoch D: " + str(stoch_d) + "\n"
                        ...
                        "Give a professional 5 sentence analysis. Mention the timeframe context, entry signal, SL/TP justification, and key risks."
                    )
                    ...
                    st.error("Could not load data for " + symbol + ". Try a different timeframe or period.")
...
with tab2:
    ...
    if st.button("Run SMC Analysis"):
        with st.spinner("Analyzing Smart Money Concepts..."):
            ...
            smc_prompt = (
                "You are a Smart Money Concepts analyst. Analyze " + ALL_PAIRS.get(smc_symbol, smc_symbol) +
                " on the " + smc_timeframe + " timeframe:\n"
                "- Order Blocks: " + ob_text + "\n"
                "- BOS/CHoCH: " + bos_text + "\n"
                "- Current Price: " + str(round(float(df["Close"].iloc[-1]), 5)) + "\n"
                "Explain what these SMC signals mean in 4-5 sentences. Mention liquidity, order blocks, and likely next move."
            )
