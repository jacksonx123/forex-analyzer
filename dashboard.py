...
with tab2:
    ...
    if st.button("Run SMC Analysis"):
        with st.spinner("Running SMC Analysis..."):
            ...
            # SMC TRADE LEVELS
            st.subheader("SMC Trade Levels")
            current_price = round(float(df["Close"].iloc[-1]), 5)
            atr_val = round(float(df["ATR"].iloc[-1]), 5)

            bull_signals = 0
            bear_signals = 0

            bull_obs = [ob for ob in all_obs if ob["type"] == "bullish"]
            bear_obs = [ob for ob in all_obs if ob["type"] == "bearish"]
            if len(bull_obs) >= len(bear_obs): bull_signals += 1
            else: bear_signals += 1

            bull_bos = [e for e in all_bos if e["direction"] == "bull"]
            bear_bos = [e for e in all_bos if e["direction"] == "bear"]
            if len(bull_bos) >= len(bear_bos): bull_signals += 1
            else: bear_signals += 1

            bull_disps = [d for d in all_disps if d["type"] == "bullish"]
            bear_disps = [d for d in all_disps if d["type"] == "bearish"]
            if len(bull_disps) >= len(bear_disps): bull_signals += 1
            else: bear_signals += 1

            bull_fvgs = [f for f in all_fvgs if f["type"] == "bullish"]
            bear_fvgs = [f for f in all_fvgs if f["type"] == "bearish"]
            if len(bull_fvgs) >= len(bear_fvgs): bull_signals += 1
            else: bear_signals += 1
            ...
            if bull_signals > bear_signals:
                ...
                bias_emoji = "🟢"
                if bull_obs:
                    entry = round((bull_obs[-1]["top"] + bull_obs[-1]["bottom"]) / 2, 5)
                elif bull_fvgs:
                    entry = round(bull_fvgs[-1]["bottom"], 5)
                else:
                    entry = current_price
                ...
                bias_emoji = "🔴"
                if bear_obs:
                    entry = round((bear_obs[-1]["top"] + bear_obs[-1]["bottom"]) / 2, 5)
                elif bear_fvgs:
                    entry = round(bear_fvgs[-1]["top"], 5)
                else:
                    entry = current_price
            else:
                smc_bias = "NEUTRAL"
            ...
            st.markdown(
                "<div style='background-color:" + bias_color + "; padding:12px; border-radius:8px; text-align:center; margin-bottom:16px;'>"                "<h2 style='color:white; margin:0;'>" + bias_emoji + " SMC BIAS: " + smc_bias + "</h2>"                "<p style='color:white; margin:4px 0 0 0; font-size:14px;'>Entry Level: <strong>" + str(entry) + "</strong> &nbsp;|&nbsp; Current Price: " + str(current_price) + "</p>"                "</div>",
                unsafe_allow_html=True
            )

            if smc_bias != "NEUTRAL":
                st.markdown("### Risk Margin Levels")
                margin_col1, margin_col2, margin_col3 = st.columns(3)

                if smc_bias == "BUY":
                    cons_sl = round(entry - atr_val * 0.8, 5)
                    cons_tp = round(entry + atr_val * 1.2, 5)
                    std_sl  = round(entry - atr_val * 1.2, 5)
                    std_tp  = round(entry + atr_val * 2.4, 5)
                    agg_sl  = round(entry - atr_val * 1.8, 5)
                    agg_tp  = round(entry + atr_val * 4.5, 5)
                else:
                    cons_sl = round(entry + atr_val * 0.8, 5)
                    cons_tp = round(entry - atr_val * 1.2, 5)
                    std_sl  = round(entry + atr_val * 1.2, 5)
                ...
                with margin_col1:
                    st.markdown(
                        "<div style='background:#1e222d; border:1px solid #2a2e39; border-radius:8px; padding:16px; text-align:center;'>"                        "<h4 style='color:#787b86; margin:0 0 10px 0;'>🛡️ CONSERVATIVE</h4>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Entry &nbsp;<strong style='color:#FFD700;'> " + str(entry) + "</strong></p>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Stop Loss &nbsp;<strong style='color:#ef5350;'> " + str(cons_sl) + "</strong></p>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Take Profit &nbsp;<strong style='color:#26a69a;'> " + str(cons_tp) + "</strong></p>"                        "<p style='color:#787b86; font-size:12px; margin:8px 0 2px 0;'>RR: 1 : 1.5 &nbsp;|&nbsp; SL = ATR×0.8</p>"                        "</div>",
                        unsafe_allow_html=True
                    )

                with margin_col2:
                    st.markdown(
                        "<div style='background:#1e222d; border:2px solid " + bias_color + "; border-radius:8px; padding:16px; text-align:center;'>"                        "<h4 style='color:" + bias_color + "; margin:0 0 10px 0;'>⚖️ STANDARD</h4>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Entry &nbsp;<strong style='color:#FFD700;'> " + str(entry) + "</strong></p>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Stop Loss &nbsp;<strong style='color:#ef5350;'> " + str(std_sl) + "</strong></p>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Take Profit &nbsp;<strong style='color:#26a69a;'> " + str(std_tp) + "</strong></p>"                        "<p style='color:#787b86; font-size:12px; margin:8px 0 2px 0;'>RR: 1 : 2 &nbsp;|&nbsp; SL = ATR×1.2</p>"                        "</div>",
                        unsafe_allow_html=True
                    )

                with margin_col3:
                    st.markdown(
                        "<div style='background:#1e222d; border:1px solid #2a2e39; border-radius:8px; padding:16px; text-align:center;'>"                        "<h4 style='color:#787b86; margin:0 0 10px 0;'>🚀 AGGRESSIVE</h4>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Entry &nbsp;<strong style='color:#FFD700;'> " + str(entry) + "</strong></p>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Stop Loss &nbsp;<strong style='color:#ef5350;'> " + str(agg_sl) + "</strong></p>"                        "<p style='color:#d1d4dc; margin:4px 0;'>Take Profit &nbsp;<strong style='color:#26a69a;'> " + str(agg_tp) + "</strong></p>"                        "<p style='color:#787b86; font-size:12px; margin:8px 0 2px 0;'>RR: 1 : 2.5 &nbsp;|&nbsp; SL = ATR×1.8</p>"                        "</div>",
                        unsafe_allow_html=True
                    )

                st.markdown("")
                st.markdown(
                    "<div style='background:#1e222d; border:1px solid #2a2e39; border-radius:6px; padding:10px; margin-top:8px;'>"                    "<p style='color:#787b86; font-size:12px; margin:0;'>⚠️ These levels are derived from SMC structure and ATR. "                    "Conservative suits low-volatility conditions. Standard is ideal for most setups. "                    "Aggressive allows for deeper pullbacks but requires higher risk tolerance. "                    "Always size positions according to your own risk management rules.</p></div>",
                    unsafe_allow_html=True
                )
            else:
                st.warning("SMC signals are mixed — no clear trade setup. Wait for structure confirmation.")
