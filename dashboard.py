...
with tab1:
    ...
    timeframe = st.selectbox("Timeframe:", list(TIMEFRAME_MAP.keys()), index=4)
    tf_config = TIMEFRAME_MAP[timeframe]
    period = st.selectbox("Period:", tf_config["periods"], index=0)
...
with tab2:
    ...
    if st.button("Run SMC Analysis"):
        with st.spinner("Running SMC Analysis..."):
            ...
            disp_text = ", ".join([d["type"] + " disp at " + str(round(d["price"], 5)) for d in all_disps]) or "None"
...
with tab3:
    ...
    bt_period = st.selectbox("Period:", bt_tf_config["periods"], index=0)
