...
def tradingview_widget(symbol, interval, height=500):
...
def get_displacement(df):
    ...
    for i in range(1, len(df)):
        ...
        if body_size > atr * 2 and move > atr * 1.5:
            ...
            displacements.append({"type": direction, "price": curr["Close"], "high": curr["High"], "low": curr["Low"], "date": df.index[i]})
...
def get_liquidity_sweeps(df, window=10):
    ...
    for i in range(window, len(df)):
        ...
        if curr["High"] > prev_high and curr["Close"] < prev_high:
            sweeps.append({"type": "buyside_sweep", "price": curr["High"], "date": df.index[i]})
        if curr["Low"] < prev_low and curr["Close"] > prev_low:
            sweeps.append({"type": "sellside_sweep", "price": curr["Low"], "date": df.index[i]})
...
with tab1:
    pairs = st.multiselect(
        ...
        list(ALL_PAIRS.keys()), default=["EURUSD=X"],
            ...
            with st.spinner("Analyzing " + ALL_PAIRS.get(symbol, symbol) + " on " + timeframe + "..."):
                try:
                    ...
                    prompt = (
                        ...
                        "5 sentence professional analysis. Include timeframe, entry justification, SL/TP reasoning, risks."
                    )
...
with tab2:
    ...
    st.markdown("TradingView chart + Order Blocks | FVGs | Displacement | Liquidity Sweeps | BOS | CHoCH")
    ...
    # Always show TradingView widget immediately
    ...
    if st.button("Run SMC Analysis"):
        with st.spinner("Running SMC Analysis..."):
            ...
            st.subheader("SMC Overlay Chart")
            fig_smc, ax = mpf.plot(
                df_plot, type="candle", style="nightclouds",
                title=ALL_PAIRS.get(smc_symbol, smc_symbol) + " — " + smc_timeframe + " SMC Overlay",
            ...
            for ob in order_blocks:
                if ob["date"] in df_plot.index:
                    ...
                    rect = mpatches.FancyBboxPatch(
                        (idx - 0.5, ob["bottom"]), len(df_plot) - idx, ob["top"] - ob["bottom"],
                        boxstyle="round,pad=0", linewidth=1.5, edgecolor=color, facecolor=color, alpha=0.2
                    )
            ...
            for fvg in fvgs:
                if fvg["date"] in df_plot.index:
                    ...
                    rect = mpatches.FancyBboxPatch(
                        (idx - 0.5, fvg["bottom"]), len(df_plot) - idx, fvg["top"] - fvg["bottom"],
                        boxstyle="round,pad=0", linewidth=1, edgecolor=color, facecolor=color, alpha=0.15, linestyle="dashed"
                    )
                    ax_main.add_patch(rect)
            ...
            for disp in displacements:
                if disp["date"] in df_plot.index:
                    ...
                    ax_main.annotate("DISP", xy=(idx, disp["price"]), fontsize=7, color=color, fontweight="bold",
            ...
            for sweep in liquidity_sweeps:
                if sweep["date"] in df_plot.index:
                    ...
                    ax_main.annotate(label, xy=(idx, sweep["price"]), fontsize=6, color="yellow", fontweight="bold",
                        arrowprops=dict(arrowstyle="-", color="yellow", lw=0.8))
                    ax_main.axhline(sweep["price"], color="yellow", linestyle=":", linewidth=0.8, alpha=0.6)
            ...
            st.markdown("""
            **Chart Legend:**
            🟢 Green zones = Bullish OB &nbsp;|&nbsp; 🔴 Red zones = Bearish OB &nbsp;|&nbsp;
            🔵 Blue dashed = Bullish FVG &nbsp;|&nbsp; 🟠 Orange dashed = Bearish FVG &nbsp;|&nbsp;
            🟡 Yellow lines = Liquidity Sweeps &nbsp;|&nbsp; **DISP** = Displacement &nbsp;|&nbsp;
            **BOS** = Break of Structure &nbsp;|&nbsp; **CHoCH** = Change of Character
            ...
            ob_text = ", ".join([ob["type"] + " OB at " + str(round(ob["top"], 5)) for ob in order_blocks]) or "None"
            fvg_text = ", ".join([f["type"] + " FVG " + str(round(f["bottom"], 5)) + "-" + str(round(f["top"], 5)) for f in fvgs]) or "None"
            disp_text = ", ".join([d["type"] + " disp at " + str(round(d["price"], 5)) for d in displacements]) or "None"
            liq_text = ", ".join([s["type"].replace("_", " ") + " at " + str(round(s["price"], 5)) for s in liquidity_sweeps]) or "None"
            bos_text = ", ".join([e["type"] + " at " + str(round(e["price"], 5)) for e in bos_choch[-3:]]) or "None"
            ...
            smc_prompt = (
                ...
                "1) What liquidity sweeps reveal about stop hunting, "
                "2) Whether displacement confirms institutional intent, "
                "3) Which order blocks are key for the next move, "
                "4) Whether FVGs need filling before continuation, "
                "5) Overall SMC bias and most likely next move."
            )
